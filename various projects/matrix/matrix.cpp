...
// ------- FUNZIONE AUSILIARIA ------------- ==> non esternata
static matrix alloc_matrix(int d1, int d2) // aka create the skeleton aka allocate the necessary memory
{
    int i;
...
matrix transpose(matrix a, int d1, int d2)
{
    int i, j;
    matrix res;
    res = alloc_matrix(d2, d1); // invertiti

    for (i = 0; i < d1; i++) // righe
    {
        for (j = 0; j < d2; j++) // colonne
        {
            res[j][i] = a[i][j];
        }
    }
