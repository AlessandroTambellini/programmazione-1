...
list *estrai(const list *l1, const list *l2);
int countInstances(const list *l, int n);
...
int countInstances(const list *l, int n)
{
  ...
  return (l->code == n ? 1 : 0) + countInstances(l->next, n);
}
...
list *estrai(const list *l1, const list *l2)
{
  ...
  if (l1->code % 2 == 1)
  {
    return new list{countInstances(l2, l1->code), estrai(l1->next, l2)};
  }
  else
  {
